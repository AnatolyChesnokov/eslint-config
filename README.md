# @anatolychesnokov/eslint-config
...
```bash
pnpm dlx @anatolychesnokov/eslint-config@latest
```
...
```bash
pnpm i -D eslint @anatolychesnokov/eslint-config
```
...
Normally you only need to import the `achesnokov` preset:
...
The `achesnokov` factory function also accepts any number of arbitrary custom config overrides:
...
export default achesnokov(
  {
    // Configures for achesnokov's config
  },
...
import {
  ...
  vue,
  yaml,
} from '@anatolychesnokov/eslint-config'
...
Check out the [configs](https://github.com/AnatolyChesnokov/eslint-config/blob/main/src/configs) and [factory](https://github.com/AnatolyChesnokov/eslint-config/blob/main/src/factory.ts) for more details.
...
Since v2.10.0, the factory function `achesnokov()` returns a [`FlatConfigComposer` object from `eslint-flat-config-utils`](https://github.com/AnatolyChesnokov/eslint-flat-config-utils#composer) where you can chain the methods to compose the config even more flexibly.
...
Use external formatters to format files that ESLint cannot handle yet (`.css`, `.html`, etc). Powered by [`eslint-plugin-format`](https://github.com/AnatolyChesnokov/eslint-plugin-format).
...
Powered by [`eslint-plugin-command`](https://github.com/AnatolyChesnokov/eslint-plugin-command). It is not a typical rule for linting, but an on-demand micro-codemod tool that triggers by specific comments.
...
- ... etc. - refer to the [documentation](https://github.com/AnatolyChesnokov/eslint-plugin-command#built-in-commands)
...
Since v3.16.0, they are no longer disabled, but made non-fixable using [this helper](https://github.com/AnatolyChesnokov/eslint-flat-config-utils#composerdisablerulesfix).
...
[MIT](./LICENSE) License &copy; 2025-PRESENT [Anatoly Chesnokov](https://github.com/AnatolyChesnokov)
